...
import com.nicos.ships.compose.application.ShipsApplication
        ...
        return when (e) {
            is HttpException -> {
                ...
                val errorMessage = responseBody?.let { getErrorMessage(it.string()) }
                    ?: getApplication<ShipsApplication>().getString(R.string.something_went_wrong)
                errorMessage
            }
            is SocketTimeoutException -> {
                getApplication<ShipsApplication>().getString(R.string.something_went_wrong_with_server)
            }
            is IOException -> {
                getApplication<ShipsApplication>().getString(R.string.check_your_internet_connection)
            }
            else -> {
                e.message ?: getApplication<ShipsApplication>().getString(R.string.something_went_wrong)
            }
        }
    }
