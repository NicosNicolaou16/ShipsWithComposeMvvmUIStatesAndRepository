...
private fun ListOfShips(
    ...
    val shipsState =
        shipsViewModel.shipsState.collectAsState(initial = ShipsState()).value
    if (shipsState.isLoading) StartDefaultLoader()
    if (!shipsState.error.isNullOrEmpty()) ShowDialog(
        title = stringResource(id = R.string.error),
        message = shipsState.error
    )
